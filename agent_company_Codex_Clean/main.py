...
async def user_input_loop():
    ...
    print()
    print("=" * 70)
    print("  AgentCompany is running. Send your request to the CEO.")
    print(f"  Active chat: {RUNTIME.current_session_name()} (#{RUNTIME.current_session_id()})")
    print("  Commands: /help, /status, /tools, /access, /vault, /upload, /chats, /quit")
    print("=" * 70)
    print()
...
async def handle_command(command: str) -> None:
    ...
    if command == "/help":
        print()
        print("Commands:")
        print("  /status  Show runtime, workspace, and agents")
        print("  /tools [on/off NAME]  Show or change tool toggles")
        print("  /access [MODE]    Show or change access mode")
        print("  /vault [PATH]     Show or set the chat-specific Obsidian vault")
        print("  /upload PATH      Upload a file or folder into the chat/workflow")
        print("  /uploads          Show upload files for the current chat")
        print("  /chats   Show saved chats")
        print("  /newchat [Name]   Create a new chat and switch to it")
        print("  /switch ID        Switch to a saved chat")
        print("  /history          Show recent messages in the active chat")
        print("  /tutorial         Start the short interactive tutorial")
        print("  /requests        Show open access/software/terminal requests")
        print("  /teams   Show active teams")
        print("  /costs   Show token/cost snapshot")
        print("  /commands     Show open terminal commands")
        print("  /approve ID   Approve and run a terminal command")
        print("  /deny ID      Deny a terminal command")
        print("  /quit    Exit")
        print()
        return
    ...
    if command == "/status":
        ...
        print(f"Active workspace: {workspace['path'] if workspace else '-'}")
        print(f"Obsidian Vault: {RUNTIME.obsidian_vault_path() or '-'}")
        print(f"Provider: {settings.PROVIDER}")
        print(f"File access: {settings.FILE_ACCESS_MODE}")
        print(f"Shell access: {settings.SHELL_ACCESS_MODE}")
        print(f"Models: CEO={settings.MODEL_CEO}, Manager={settings.MODEL_MANAGER}, Worker={settings.MODEL_WORKER}")
        print()
        return
    ...
    if command == "/costs":
        ...
        print(f"Estimated cost: ${snap['total_usd']:.6f}")
        for agent_id, row in sorted(snap["by_agent"].items()):
            print(f"- {agent_id}: {row['calls']} calls, ${row['usd']:.6f}, model={row['model']}")
        print()
        return
...
async def change_access_mode_from_chat(raw_args: str) -> None:
    ...
    print()
    print("Access mode updated.")
    print(_access_status_text())
    if _full_access_enabled():
        print()
        _print_red_block([
            "WARNING: full access is active. This beta program can modify files",
            "         and execute commands automatically.",
        ])
    print()
...
async def print_tool_toggles() -> None:
    ...
    for name, enabled in settings.tool_toggle_snapshot().items():
        ...
        print(f"  - {name}: {status}")
    print("Change with: /tools on NAME or /tools off NAME")
...
async def run_interactive_tutorial(first_run: bool = False) -> None:
    ...
    print("=" * 70)
    if first_run:
        print("Welcome. I will quickly walk you through the most important parts.")
    else:
        print("Here is the short interactive introduction.")
    print()
    print("1. You talk directly to the CEO here.")
    print("2. Use /newchat to create separate chats.")
    print("3. Use /vault to set a chat-specific Obsidian vault.")
    print("4. Use /upload to add files or folders to the current chat.")
    print("5. Use /status to see provider, workspace, and access mode.")
    print("6. Use /tools to enable or disable tool groups.")
    print()

    wants_access = await _prompt_yes_no(
        "Do you want to configure the access mode now? [Y/n]: ",
        default=True,
    )
    if wants_access:
        ...
        print()
        print("All right. You can change the mode later at any time with /access.")
        print()
    ...
    print("  /tools on zip")
    print("  /vault /Users/yourname/Documents/MyVault")
    print("  /upload /Users/yourname/Desktop/file.txt")
    print("  /newchat My test project")
    print()
    if first_run:
        print("First launch is now marked complete. The tutorial remains available with /tutorial.")
    print("=" * 70)
    print()
...
async def _interactive_access_setup() -> None:
    print()
    print("Access modes:")
    print("  1) approval        - files and shell require approval")
    print("  2) full            - files and shell run automatically")
    print("  3) approval full   - files require approval, shell runs automatically")
    print("  4) full approval   - files run automatically, shell requires approval")
    print("  5) keep current mode")
    print()

    raw = await _prompt_text("Choice [1-5]: ")
    ...
    if choice == "5":
        print()
        print("Access mode stays unchanged.")
        print(_access_status_text())
        print()
        return
    if choice not in mapping:
        print()
        print("Invalid choice. Access mode stays as it is.")
        print(_access_status_text())
        print()
        return
    ...
    refresh_agent_tools()
    print()
    print("Access mode saved.")
    print(_access_status_text())
    if _full_access_enabled():
        print()
        _print_red_block([
            "WARNING: full access is risky in a beta program.",
            "         Possible damage includes modified files or",
            "         automatically executed shell commands.",
        ])
    print()
...
async def set_obsidian_vault(raw_path: str) -> None:
    ...
    if not os.path.isdir(path):
        print(f"Vault folder not found: {path}")
        return
    db = get_managers_db()
    await db.update(
        "chat_sessions",
...
async def upload_path_to_chat(raw_path: str) -> None:
    ...
    await db.insert("user_messages", {
        ...
        "content": (
            ...
            f"Original path: {src}\n"
            f"Stored at: {dest}\n"
            "Use this file as context for the current task."
        ),
    })
    print(f"\n{kind} uploaded to: {dest}\n")
