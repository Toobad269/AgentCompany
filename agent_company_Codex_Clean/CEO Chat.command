...
if ! python3 settings.py; then
  echo
  echo "Provider-Setup ist noch nicht korrekt eingerichtet."
  echo "Wir richten jetzt standardmaessig OpenAI ein."
  python3 settings.py setup openai || exit 1
  echo
  python3 settings.py || {
    echo
    echo "Setup ist noch nicht korrekt. Fenster bleibt offen."
...
echo
read -r -p "CEO Chat beendet. Enter druecken zum Schliessen ..."
