...
PLAN_PROFILES = {
    "free": {
        ...
        "price_monthly_eur": 0,
        "description": "Basic chat and file tests. No always-on cloud automations.",
        "max_teams": 1,
        "max_workers_per_team": 2,
        "default_workers_per_team": 1,
        "always_on_automations": 0,
        "checks_per_day": 0,
        "ai_actions_monthly": 50,
        "storage_gb": 0.5,
        "tools": {"uploads", "pdf"},
        "highlights": [
            "0 always-on automations",
            "50 AI actions/month",
            "Uploads and PDF tools",
        ],
    },
    "starter": {
        "label": "Starter",
        "tagline": "Solo projects",
        "price_monthly_eur": 9,
        "description": "Small projects with one background automation and hourly checks.",
        "max_teams": 2,
        "max_workers_per_team": 3,
        "default_workers_per_team": 2,
        "always_on_automations": 1,
        "checks_per_day": 24,
    ...
    "pro": {
        "label": "Pro",
        "tagline": "Research & code",
        "price_monthly_eur": 29,
        "description": "Regular research and coding workflows with multiple always-on automations.",
        "max_teams": 5,
        "max_workers_per_team": 6,
        "default_workers_per_team": 4,
        "always_on_automations": 5,
        "checks_per_day": 250,
        "ai_actions_monthly": 3000,
        "storage_gb": 10,
    ...
    "business": {
        "label": "Business",
        "tagline": "Team workflows",
        "price_monthly_eur": 79,
        "description": "Larger teams, more storage, and GitHub write access for serious workflows.",
        "max_teams": 8,
        "max_workers_per_team": 10,
        "default_workers_per_team": 5,
        "always_on_automations": 20,
        "checks_per_day": 1500,
        "ai_actions_monthly": 12000,
        "storage_gb": 50,
        "tools": set(TOOL_TOGGLE_NAMES),
        "highlights": [
            "20 always-on automations",
            "1,500 checks/day",
            "12,000 AI actions/month",
        ],
    },
    "custom": {
        "label": "Custom",
        "tagline": "Custom limits",
        "price_monthly_eur": None,
        "description": "For heavy always-on workloads. Limits and pricing are agreed individually.",
        "max_teams": 20,
        "max_workers_per_team": 20,
        "default_workers_per_team": 8,
        "always_on_automations": 100,
        "checks_per_day": 10000,
        "ai_actions_monthly": 100000,
        "storage_gb": 250,
        "tools": set(TOOL_TOGGLE_NAMES),
        "highlights": [
            "Custom always-on capacity",
            "Custom AI action budget",
            "Priority setup required",
        ],
    },
}

PLAN_ALIASES = {"plus": "pro", "studio": "business"}
...
SUBSCRIPTION_PLAN = _config_value("SUBSCRIPTION_PLAN", "business").strip().lower() or "business"
SUBSCRIPTION_PLAN = PLAN_ALIASES.get(SUBSCRIPTION_PLAN, SUBSCRIPTION_PLAN)
if SUBSCRIPTION_PLAN not in PLAN_PROFILES:
    SUBSCRIPTION_PLAN = "business"
PLAN_SELECTED = _config_value("PLAN_SELECTED", "0").strip() == "1"
...
def _validate():
    if PROVIDER == "openai" and not API_KEY:
        raise RuntimeError(
            ...
            "   Set it securely with: python3 settings.py setup openai\n"
            "   Or temporarily export it with: export OPENAI_API_KEY='...'\n"
        )
    if PROVIDER == "ollama" and not API_BASE_URL:
        raise RuntimeError(
            "\n\n❌ OLLAMA_API_BASE_URL is missing.\n"
            "   The default is usually: http://localhost:11434/v1\n"
        )
    if OBSIDIAN_VAULT_PATH and not os.path.isdir(OBSIDIAN_VAULT_PATH):
        raise RuntimeError(
            "\n\n❌ OBSIDIAN_VAULT_PATH does not point to a folder.\n"
            f"   Current value: {OBSIDIAN_VAULT_PATH}\n"
        )
    if FILE_ACCESS_MODE not in ACCESS_MODES:
        raise RuntimeError(
            "\n\n❌ FILE_ACCESS_MODE is invalid.\n"
            "   Allowed: approval | full\n"
        )
    if SHELL_ACCESS_MODE not in ACCESS_MODES:
        raise RuntimeError(
            "\n\n❌ SHELL_ACCESS_MODE is invalid.\n"
            "   Allowed: approval | full\n"
        )
...
def _write_env(provider: str, api_key: str = "") -> None:
    if provider == "openai":
        content = [
            "# Local secrets for AgentCompany",
...
def _setup_interactive(provider: str) -> None:
    if provider == "openai":
        ...
        print("The key will be requested securely and stored locally in .env.")
        api_key = getpass.getpass("API-Key: ").strip()
        if not api_key:
            print("Cancelled: empty API key.")
            return
        _write_env(provider, api_key=api_key)
    else:
        print("Set up Ollama mode")
        print("No external API key is required.")
        print("Requirement: Ollama must be running locally and a model like devstral must be pulled.")
        _write_env(provider)
    print("✅ .env saved. Test it now with: python3 settings.py")
...
def _set_access_modes(file_mode: str, shell_mode: str) -> None:
    ...
    if file_mode not in ACCESS_MODES or shell_mode not in ACCESS_MODES:
        print("\n❌ Invalid mode. Allowed values: approval | full\n")
        raise SystemExit(1)
    ...
    print("✅ Access modes saved.")
    print(f"   File access: {file_mode}")
    print(f"   Shell access: {shell_mode}")
    if "full" in {file_mode, shell_mode}:
        print()
        print("WARNING: full access is risky in a beta program.")
        print("         Files may be modified and commands may run automatically.")
        print("         Use this only if you trust the project and prompts.")
...
def reload_runtime_settings() -> None:
    ...
    global SUBSCRIPTION_PLAN, PLAN_SELECTED, MAX_TEAMS, MAX_WORKERS_PER_TEAM, DEFAULT_WORKERS_PER_TEAM

    _ENV_FILE = _read_env_file()

    sp = _config_value("SUBSCRIPTION_PLAN", SUBSCRIPTION_PLAN).strip().lower() or SUBSCRIPTION_PLAN
    sp = PLAN_ALIASES.get(sp, sp)
    if sp in PLAN_PROFILES:
        SUBSCRIPTION_PLAN = sp
        prof = PLAN_PROFILES[SUBSCRIPTION_PLAN]
        MAX_TEAMS = int(prof["max_teams"])
        MAX_WORKERS_PER_TEAM = int(prof["max_workers_per_team"])
        DEFAULT_WORKERS_PER_TEAM = int(prof["default_workers_per_team"])
    PLAN_SELECTED = _config_value("PLAN_SELECTED", "0").strip() == "1"
...
def save_access_modes(file_mode: str, shell_mode: str) -> None:
    ...
    shell_mode = shell_mode.strip().lower()
    if file_mode not in ACCESS_MODES or shell_mode not in ACCESS_MODES:
        raise ValueError("Invalid mode. Allowed values: approval | full")
    ACCESS_MODE = "full" if file_mode == "full" and shell_mode == "full" else "approval"
    FILE_ACCESS_MODE = file_mode
    SHELL_ACCESS_MODE = shell_mode
    _write_env_values({
...
def save_llm_config(
    *,
    ...
    if not model_ceo or not model_manager or not model_worker:
        raise ValueError("All three model fields must be set.")
    ...
    if invalid_reasoning:
        raise ValueError("Reasoning must be minimal, low, medium, or high.")
...
def plan_tool_allowed(name: str) -> bool:
    if name not in TOOL_TOGGLE_NAMES:
        raise ValueError(f"Unknown tool toggle: {name}")
...
def current_plan_payload() -> dict[str, object]:
    ...
    for slug, item in PLAN_PROFILES.items():
        plans.append({
            ...
            "price_monthly_eur": item.get("price_monthly_eur", 0),
            ...
            "always_on_automations": item["always_on_automations"],
            "checks_per_day": item["checks_per_day"],
            "ai_actions_monthly": item["ai_actions_monthly"],
            "storage_gb": item["storage_gb"],
            "max_agents": int(item["max_teams"]) * int(item["max_workers_per_team"]),
    ...
    return {
        ...
        "price_monthly_eur": profile.get("price_monthly_eur", 0),
        ...
        "always_on_automations": profile["always_on_automations"],
        "checks_per_day": profile["checks_per_day"],
        "ai_actions_monthly": profile["ai_actions_monthly"],
...
def mark_plan_selected() -> None:
    global PLAN_SELECTED
    PLAN_SELECTED = True
    _write_env_values({"PLAN_SELECTED": "1"})


def save_subscription_plan(plan_name: str, force: bool = False) -> None:
    """Speichert den Plan. Wechsel sind jederzeit möglich (Up- oder Downgrade).
    Der Parameter `force` existiert nur aus historischen Gründen."""
    ...
    if plan_name not in PLAN_PROFILES:
        raise ValueError("Unknown plan. Allowed: free, starter, pro, business, custom")
...
def save_tool_toggle(name: str, enabled: bool) -> None:
    ...
    if enabled and not plan_tool_allowed(name):
        ...
        raise ValueError(f"'{name}' is not unlocked in the active plan {plan_label}.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if command.startswith("sk-") or command == "set-key":
            print(
                "\n❌ Please do not pass the API key as a command-line argument.\n"
                "   It would end up visible in your shell history.\n"
                "   Use this instead: python3 settings.py setup openai\n"
            )
            raise SystemExit(1)

    try:
        _validate()
        print("✅ settings.py is configured correctly.")
        print(f"   Provider:       {PROVIDER}")
        print(f"   API URL:        {API_BASE_URL}")
        print(f"   CEO model:      {MODEL_CEO}")
        print(f"   Manager model:  {MODEL_MANAGER}")
        print(f"   Worker model:   {MODEL_WORKER}")
        ...
        print(f"   File access:    {FILE_ACCESS_MODE}")
        print(f"   Shell access:   {SHELL_ACCESS_MODE}")
        print(
            "   Tools:          "
            + ", ".join(
        ...
        print(f"   Max workers/team:{MAX_WORKERS_PER_TEAM}")
