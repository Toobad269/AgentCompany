...
import os, sys, json, sqlite3, glob, shutil, time, io, zipfile, mimetypes
...
def _queue_action(action: str, payload: dict | None = None) -> int:
    ...
    try:
        ...
        abort(503, "main.py is not active (web_actions table is missing)")
...
def api_plans():
    ...
    return jsonify(settings.current_plan_payload())
...
def api_plan_select():
    ...
    if plan not in settings.PLAN_PROFILES:
        return jsonify({"error": f"Unbekannter Plan: {plan}"}), 400
    was_selected = settings.PLAN_SELECTED
    try:
        settings.save_subscription_plan(plan)
        if not was_selected:
            settings.mark_plan_selected()
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
    aid = _queue_action("refresh_tools", {})
    _wait_action(aid, timeout=2.0)
    return jsonify({"plan": plan, "selected": True, "changed": was_selected})
...
def api_system():
    ...
    plan_payload = settings.current_plan_payload()
    plan_allowed = {n: settings.plan_tool_allowed(n) for n in settings.TOOL_TOGGLE_NAMES}
    return jsonify({
...
def _safe_under(parent: str, child: str) -> bool:
    """True wenn child wirklich unter parent liegt (kein Pfad-Escape)."""
    p = os.path.realpath(parent)
    c = os.path.realpath(child)
    return c == p or c.startswith(p + os.sep)


def _zip_dir_to_bytes(folder: str) -> io.BytesIO:
    buf = io.BytesIO()
    base = os.path.basename(folder.rstrip(os.sep)) or "archive"
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(folder):
            # Lokale Junk-Dateien überspringen
...
def api_workspaces():
    ...
    if sid is not None:
        ...
        params = (sid,)
    rows = _q(_mgr(), f"SELECT * FROM workspaces {where} ORDER BY id DESC", params)
    out = []
    for r in rows:
        path = r.get("path") or ""
        out.append({
...
@app.route("/api/workspaces/<int:wid>/download")
def api_workspace_download(wid):
    rows = _q(_mgr(), "SELECT * FROM workspaces WHERE id = ?", (wid,))
    if not rows:
        return jsonify({"error": "Workspace nicht gefunden"}), 404
    ws = rows[0]
    path = ws.get("path") or ""
    if not path or not os.path.isdir(path):
        return jsonify({"error": "Workspace-Ordner existiert nicht"}), 404
    # Sicherheit: muss unter dem WORKSPACE_DIR liegen
    if not _safe_under(settings.WORKSPACE_DIR, path):
        return jsonify({"error": "Pfad außerhalb des Workspace-Bereichs"}), 400

    buf = _zip_dir_to_bytes(path)
    name = (ws.get("short_name") or f"workspace_{wid}").strip().replace("/", "_") or f"workspace_{wid}"
    fname = f"{name}.zip"
    return send_file(buf, mimetype="application/zip", as_attachment=True, download_name=fname)


@app.route("/api/uploads/download")
def api_upload_download():
    """Einzelne Upload-Datei oder ganzen Upload-Ordner als ZIP herunterladen."""
    sid = request.args.get("session_id", type=int) or _current_session_id()
    ...
    if not name:
        # Gesamten Upload-Ordner als ZIP
        buf = _zip_dir_to_bytes(root)
        return send_file(buf, mimetype="application/zip", as_attachment=True,
                         download_name=f"uploads_session_{sid}.zip")

    safe = os.path.basename(name)
    full = os.path.join(root, safe)
    if not _safe_under(root, full) or not os.path.exists(full):
        return jsonify({"error": "Datei nicht gefunden"}), 404

    if os.path.isdir(full):
        buf = _zip_dir_to_bytes(full)
        return send_file(buf, mimetype="application/zip", as_attachment=True,
                         download_name=f"{safe}.zip")

    mime = mimetypes.guess_type(full)[0] or "application/octet-stream"
    return send_file(full, mimetype=mime, as_attachment=True, download_name=safe)
...
def api_chat_vault():
    ...
    if vault and not os.path.isdir(os.path.expanduser(vault)):
        return jsonify({"error": f"Folder not found: {vault}"}), 400
...
def api_command_approve(cid):
    ...
    if rows[0]["status"] != "pending":
        return jsonify({"error": f"Command is {rows[0]['status']}"}), 400
...
def api_command_deny(cid):
    ...
    _exec(
        ...
        (_now(), rows[0].get("session_id"),
         ...
         f"User denied terminal command #{cid}: {rows[0]['command']}"),
    )
