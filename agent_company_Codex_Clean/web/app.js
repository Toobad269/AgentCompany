  ...
  mscroll.innerHTML = msgs.length
    ? msgs.map(m => `
        <div class="bubble-row ${m.direction}">
          ...
          <div class="bubble-meta">${m.direction === 'in' ? 'You' : 'CEO'} · ${ago(m.created_at)}</div>
        </div>`).join('')
    : `<div class="empty"><div class="empty-icon">✨</div><div class="empty-title">Give the CEO a task</div></div>`;
...
function renderSystem() {
  ...
  const planCard = `<div class="card">
    ...
    <div class="kv-row"><span class="kv-key">Always-on</span><span class="kv-val">${planP.always_on_automations ?? '–'}</span></div>
    <div class="kv-row"><span class="kv-key">Checks/Tag</span><span class="kv-val">${planP.checks_per_day ?? '–'}</span></div>
    <div class="kv-row"><span class="kv-key">AI-Aktionen/Monat</span><span class="kv-val">${planP.ai_actions_monthly ?? '–'}</span></div>
    <div class="kv-row"><span class="kv-key">Speicher</span><span class="kv-val">${planP.storage_gb ?? '–'} GB</span></div>
  ...
  grid.innerHTML = payload.plans.map(p => {
    ...
    <div class="plan-card ${p.active ? 'active' : ''}" onclick="selectPlan('${p.slug}', this)">
      ...
      <div class="plan-stats">
        ...
        <span class="plan-stat">${p.always_on_automations ?? 0} Always-on</span>
        <span class="plan-stat">${p.checks_per_day ?? 0} Checks/Tag</span>
        <span class="plan-stat">${p.ai_actions_monthly ?? 0} AI-Aktionen/Monat</span>
        <span class="plan-stat">${p.storage_gb ?? 0} GB Speicher</span>
      </div>
