...
function renderChat() {
  ...
  if (!S.dbLive) {
    slist.innerHTML = '';
    mscroll.innerHTML = empty('💤', 'AgentCompany läuft noch nicht',
      'Starte main.py — das Dashboard verbindet sich automatisch');
    meta.textContent = 'Kein Backend';
    return;
  }
  if (!S.sessions.length) {
    slist.innerHTML = `<button class="btn" style="width:100%" onclick="newChatPrompt()">＋ Ersten Chat anlegen</button>`;
    mscroll.innerHTML = empty('💬', 'Noch kein Chat',
      'Klick links auf „＋", um den ersten Chat anzulegen');
    meta.textContent = 'Kein aktiver Chat';
    return;
  }
  ...
  slist.innerHTML = S.sessions.map(s => `
    <div class="session-item ${s.id === sid ? 'active' : ''}" onclick="selectSess(${s.id})">
      ...
      <button class="session-del" title="Chat löschen" onclick="deleteChat(event, ${s.id})">✕</button>
    </div>`).join('');
  ...
  document.getElementById('vault-current').textContent = 'Aktuell: ' + (active.vault_path || '—');

  // Composer-Meta
  meta.textContent = `Aktiver Chat: ${active.name} (#${active.id})`;
  ...
  mscroll.innerHTML = msgs.length
    ? msgs.map(m => `
        <div class="bubble-row ${m.direction}">
          ...
          <div class="bubble-meta">${m.direction === 'in' ? 'Du' : 'CEO'} · ${ago(m.created_at)}</div>
        </div>`).join('')
    : `<div class="empty"><div class="empty-icon">✨</div><div class="empty-title">Schreib dem CEO eine Aufgabe</div></div>`;
...
async function deleteChat(ev, id) {
  ...
  if (!confirm(`Chat "${name}" wirklich löschen?\nAlle Nachrichten und Uploads dieses Chats werden entfernt.`)) return;
  try {
    await api('DELETE', `/api/chat/${id}`);
    if (S.activeSess === id) S.activeSess = null;
    toast(`Chat "${name}" gelöscht`, 'ok');
    poll();
  } catch (e) { toast(e.message, 'err'); }
}

async function newChatPrompt() {
  const name = prompt('Name für den neuen Chat:', '');
  if (name === null) return;
  try {
    const r = await api('POST', '/api/chat/new', { name });
    S.activeSess = r.id;
    toast(`Chat "${r.name}" angelegt`, 'ok');
    poll();
  } catch (e) { toast(e.message, 'err'); }
}
...
async function setVault() {
  ...
  if (!sid) { toast('Kein Chat aktiv', 'err'); return; }
  try {
    await api('POST', '/api/chat/vault', { session_id: sid, vault_path: v });
    toast('Vault gesetzt', 'ok');
    poll();
  } catch (e) { toast(e.message, 'err'); }
}
...
async function uploadFileObj(f) {
  if (!S.activeSess) { toast('Kein Chat aktiv', 'err'); return; }
  ...
  try {
    ...
    toast(`Hochgeladen: ${data.name}`, 'ok');
    poll();
  } catch (e) { toast(e.message, 'err'); }
}
...
function renderFeed() {
  ...
  if (!evs.length) {
    document.getElementById('ev-list').innerHTML = empty('📡', 'Keine Ereignisse');
    return;
  }
...
function renderOrg() {
  ...
  if (!S.dbLive) { el.innerHTML = empty('🏢', 'AgentCompany läuft noch nicht'); return; }
  ...
  const teamsHtml = S.org.length
    ? S.org.map(t => {
        ...
        return `<div class="org-team-card" onclick="openTeam(${t.id})">
          ...
          <div class="org-stats">
            ...
            ${t.results_count ? `<span class="org-stat g">${t.results_count} Ergebnisse</span>` : ''}
          </div>
    ...
    : `<div style="color:var(--muted);font-size:12px;text-align:center;padding:24px">
         Der CEO erstellt Teams automatisch sobald er eine Aufgabe bekommt.
       </div>`;
  ...
  el.innerHTML = `
    <div class="org-wrap">
      <div class="org-ceo-box">
        ...
        <div class="org-ceo-sub">plant · delegiert · antwortet</div>
      </div>
...
function renderTeams() {
  ...
  if (!S.org.length)   { document.getElementById('team-detail').innerHTML = empty('👥', 'Noch keine Teams'); return; }
  if (!S.activeTeam)   { document.getElementById('team-detail').innerHTML = `<div style="color:var(--muted);font-size:13px;padding:14px">← Team wählen</div>`; return; }
  if (S.teamDetail)    renderTeamDetail(S.teamDetail);
}

function renderTeamDetail(d) {
  const briefingHtml = d.briefing
    ? `<div class="card"><div class="card-label">📋 Briefing vom CEO</div>
  ...
  const tasksHtml = (d.tasks || []).map(t => `
    ...
    </div>`).join('') || `<div style="color:var(--muted);padding:8px;font-size:12px">Keine Tasks</div>`;
  ...
  }).join('') || `<div style="color:var(--muted);padding:8px;font-size:12px">Kein Chat</div>`;
  ...
  const resultsHtml = (d.results || []).map(r => `
    ...
    </div>`).join('') || `<div style="color:var(--muted);padding:8px;font-size:12px">Noch keine Ergebnisse</div>`;
  ...
  document.getElementById('team-detail').innerHTML = briefingHtml + `
    <div class="team-3col">
      ...
      <div class="tcol"><div class="tcol-head">✅ Ergebnisse (${(d.results || []).length})</div><div class="tcol-body">${resultsHtml}</div></div>
    </div>`;
...
function renderCommands() {
  ...
  if (!S.commands.length) {
    el.innerHTML = empty('⚡', 'Keine Terminal-Befehle', 'Hier erscheinen Befehle die der CEO anfragen möchte');
    return;
  }
  ...
  if (pending.length) {
    html += `<div class="pending-banner">
      <div class="pending-title">⚡ ${pending.length} Befehl${pending.length > 1 ? 'e' : ''} warte${pending.length > 1 ? 'n' : ''} auf deine Freigabe</div>
      <div class="pending-hint">Klicke direkt auf <b>Approve</b> oder <b>Deny</b>.</div>
    </div>`;
  }
...
async function approveCmd(id, btn) {
  ...
  try {
    ...
    toast(r.result || 'Befehl ausgeführt', r.status === 'error' ? 'err' : 'ok');
    poll();
...
function renderRequests() {
  ...
  const sec = (title, items, render) => `
    <div class="req-section">
      ...
      ${items.length ? items.map(render).join('') : `<div style="color:var(--muted);font-size:12px;padding:8px 4px">Keine offenen Anfragen.</div>`}
    </div>`;

  const html =
    sec('⚡ Terminal-Befehle', r.terminal, x => `
      <div class="req-item">
        <div class="req-body">
...
function renderSystem() {
  ...
  if (!S.system) { el.innerHTML = empty('⚙️', 'Lade System…'); return; }
  const sy = S.system;
  ...
  const accessCard = `<div class="card">
    <div class="card-label">🔐 Zugriffsmodi</div>
    <div class="access-grid">
      <div class="access-key">Dateien</div>
    ...
    ${(sy.file_access_mode === 'full' || sy.shell_access_mode === 'full') ?
      `<div class="muted-mini" style="color:var(--red);margin-top:6px">⚠️ Vollzugriff aktiv — Agenten dürfen Dateien ändern bzw. Shell-Befehle direkt ausführen.</div>` : ''}
  </div>`;

  const toolsRows = Object.keys(sy.tools).sort().map(name => `
    <div class="toggle-row">
      <span class="toggle-name">${esc(name)}</span>
      <div class="switch ${sy.tools[name] ? 'on' : ''}" onclick="toggleTool('${name}', ${!sy.tools[name]})"></div>
    </div>`).join('');
  ...
  const costsCard = `<div class="card">
    ...
    <div class="kv-row"><span class="kv-key">Geschätzte Kosten</span><span class="kv-val">$${(cs.total_usd || 0).toFixed(6)}</span></div>
    <div style="margin-top:10px">${byAgentRows}</div>
  </div>`;

  el.innerHTML = `<div class="sys-grid">${statusCard}${accessCard}${toolsCard}${costsCard}</div>`;
}
...
function renderUploads() {
  ...
  const list = u.files && u.files.length
    ...
    : empty('📎', 'Keine Uploads', 'Lade über das Feld oben oder das 📎 im Chat hoch');

  el.innerHTML = drop + list;
}
...
async function poll() {
  try {
    ...
    if (S.view === 'uploads') await pollUploads();
