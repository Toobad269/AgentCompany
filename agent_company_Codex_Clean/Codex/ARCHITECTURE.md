...
- **UI:** zwei gleichwertige Front-Ends — Terminal-Chat über `main.py`
  und Web-UI über `webapp.py` (Port 7842). Beide nutzen denselben
  Agent-Runtime und dieselben DBs.
...
├── webapp.py            # Entry-Point Web-UI (Port 7842)
├── CEO Chat.command     # macOS-Starter Terminal-UI
├── Web Chat.command     # macOS-Starter Web-UI
├── core/
...
│   ├── manager.py
│   └── worker.py
├── web/                 # Statische Assets der Web-UI
├── databases/           # SQLite-Files (zur Laufzeit erzeugt)
├── memory/              # Langzeit-Memory pro Agent (Markdown)
├── workspaces/          # Arbeitsordner pro User-Auftrag
├── incoming_files/      # Uploads aus dem Chat
└── requirements.txt
```
